...
#(
  ...
  if (ShimEnable) begin
    ...
    tlul_adapter #(
      ...
      .tl_i         ( tl_d2h_delayed      ),
      .tl_o         ( tl_h2d              ),
      .tlul_en_i    ( ~bus_req.c_dpi_load ),
      .tlul_wait_o  ( bus_wait            ),
      .tlul_addr_i  ( bus_req.addr        ),
      .tlul_write_i ( bus_req.write       ),
      .tlul_wdata_i ( bus_req.wdata       ),
      .tlul_rdata_o ( bus_rdata           ),
      .tlul_error_o ( bus_error           )
    );
  end
  ...
  // counter is reset for every `c_dpi_load` request, i.e., bus_req.dv = 0.
