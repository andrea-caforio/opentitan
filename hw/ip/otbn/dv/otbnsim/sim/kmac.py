...
(before the instruction executes) and commit() is called at the end of every cycle (after the
instruction commits or aborts).
'''
...
class Kmac:
        ...
        match self._state:
            ...
            case _State.WAIT_FOR_MSG:
                if cmd_send_issued and not self._no_more_msg_allowed:
                    ...
                    if not self._service_rejected:
                        ...
                        # channel.
                        self._start_digest()
                        # The process round runs after any absorb permutation still in flight.
            ...
            case _State.RECEIVING:
                if cmd_done_issued:
                    ...
                    # RSP_VALID asserted into the next session (matches the RTL clear_rsp_valid,
                    # which also clears data_sx_pending).
    ...
    def detect_errors(self) -> None:
        ...
        commits KMAC_STATUS. It runs on both paths because the error bits are sticky and commit
        even if the instruction aborts.'''

        # CTRL_ERROR: A command issued out of order for the current state, or a command write with
        # multiple command bits set at once.
    ...
    def _send_beat(self, idx: int) -> tuple[bool, bool, bool]:
        '''Send one 64-bit message beat (index idx) from the staged WSR.
    ...
    def _decode_config(self) -> bool:
        # The upper fields must hold the bitwise inverted value of the lower fields.
