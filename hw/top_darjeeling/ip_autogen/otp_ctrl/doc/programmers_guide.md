...
For example, the digest field is never writable through the `WRITE` command in a hardware partition.
    ...
    > Side effect: The `otp_ctrl` detects the first _successful_ word zeroization and disables periodic consistency checks for the corresponding partition as these can potentially fail when interrupting an ongoing zeroization procedure.
...
2. A successful zeroization of a fuse word results in the number of set bits in the zeroized workd being returned to software in the `DIRECT_ACCESS_RDATA` registers for inspection bypassing the
...
3. When firmware determines that a partition has been sufficiently zeroized, it should reset the `otp_ctrl` such that the zeroized data is also reflected in the buffer registers.

     > Side effect: Upon initialization, the `otp_ctrl` will first read the zeroization field of a partition to determine whether it is in a zeroized state.
