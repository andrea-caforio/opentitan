...
The zeroization flow needs to be supported by the attached `otp_macro` ideally with a dedicated command.
...
Furthermore, as software needs to verify for each word that it was successfully zeroized, the `otp_macro` macro has to return the zeroized word back to the `otp_macro` which will further process it before releasing it to software for inspection.
...
There are two distinct cases when the `otp_ctrl` performs a zeroization detection by counting the number of set bits in a word and comparing the resulting value against predefined thresholds that account for potentially stuck-at-0 bits.
...
The `otp_ctrl` detects the first successful zeroization of a word in a partition and disables any further periodic or manually triggered consistency checks.
The integrity checks can proceeds as they only act on already buffered data.
After the `otp_ctrl` is reset both integrity and consistency checks are disabled for partitions that are marked as zeroized by way of the zeroization marker field.
