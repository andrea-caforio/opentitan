...
def _preprocess_source(ctx, src):
    """Run the C preprocessor on `src` and return the resulting .s file."""
    cc_toolchain = find_cc_toolchain(ctx)

    out = ctx.actions.declare_file(
        "{}_pp/{}.pp.s".format(ctx.attr.name, src.basename),
    )
    ...
    ctx.actions.run(
        ...
        arguments = ["-E", "-x", "assembler-with-cpp", "-nostdinc",
                     src.path, "-o", out.path],
        executable = ctx.executable._riscv32_clang,
    )
...
def _otbn_assemble_sources(ctx, additional_srcs = []):
    ...
    corresponding object file by assembling the provided src files. Each
    source is first run through the C preprocessor.
    ...
    raw_srcs = ctx.files.srcs + additional_srcs
    pp_srcs = [_preprocess_source(ctx, s) for s in raw_srcs]
    paths = [s.path for s in pp_srcs]

    ctx.actions.run(
        outputs = [obj],
        inputs = depset(
            direct = pp_srcs + [ctx.executable._otbn_as],
            transitive = [cc_toolchain.all_files],
        ),
        env = {
