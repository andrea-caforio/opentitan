...
static status_t run_mldsa87_verify(const mldsa87_verify_testvector_t *vector) {
  ...
  otcrypto_const_word32_buf_t sig = OTCRYPTO_MAKE_BUF(otcrypto_const_word32_buf_t, vector->sig, kOtcryptoMldsa87SigWords);
  otcrypto_const_byte_buf_t msg = OTCRYPTO_MAKE_BUF(otcrypto_const_byte_buf_t, vector->msg, vector->msg_len);
  otcrypto_const_byte_buf_t ctx = OTCRYPTO_MAKE_BUF(otcrypto_const_byte_buf_t, vector->ctx, vector->ctx_len);

  hardened_bool_t verification_result;  
  CHECK_STATUS_OK(otcrypto_mldsa87_verify(&pk, &msg, &ctx, &sig, vector->hash_mode, &verification_result));
...
static status_t mldsa87_verify_pure_test(void) {
  return run_mldsa87_verify(&mldsa87_verify_testvectors[kMldsa87VerifyNistAcvp63Pure]);
}
...
static status_t mldsa87_verify_pre_hash_test(void) {
  return run_mldsa87_verify(&mldsa87_verify_testvectors[kMldsa87VerifyNistAcvp79PreHashSha3_224]);
}
