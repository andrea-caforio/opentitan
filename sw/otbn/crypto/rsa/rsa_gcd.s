 ...
 *
 * @param[in]  x20: dptr_x, pointer to first limb of x in dmem
 ...
 *
 * @param[in]  x20: dptr_x, pointer to first limb of x in dmem
 * @param[in]  x21: plen, number of 256-bit limbs for x
 ...
 *
 * @param[in]   x26: dptr_a, pointer to input a in DMEM
 * @param[in]   x27: dptr_b, pointer to input b in DMEM
 ...
 * Flags have no meaning beyond the scope of this subroutine.
 *
 * @param[in]     x20: dptr_x, pointer to input x in DMEM
 * @param[in]     x21: dptr_y, pointer to input y in DMEM
 * @param[in]     x30: n, number of 256-bit limbs for x, y, and g
 * @param[in]     w31: all-zero
 * @param[out] dmem[dptr_y:dptr_y+n*32]: g, result
 *
 * clobbered registers: x21 to x27, w20 to w25
...
gcd:
  ...
  loop      x22, 30
    /* Load the least significant limbs of x and y.
         w23 <= dmem[x20] = x[255:0]
         w24 <= dmem[x21] = y[255:0] */
    bn.lid    x23, 0(x20)
    bn.lid    x24, 0(x21)
