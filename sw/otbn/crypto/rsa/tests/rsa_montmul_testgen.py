...
@testcase
def gen_montmul_test(seed: Optional[int] = None):
    ...
    x = random.randint(0, 2**mod_size-1)
    y = random.randint(0, 2**mod_size-1)
    z = mont(x, y, mu, n, mod_size)
    ...
    return {
        ...
        "input": {
            ...
            "dmem": {
                "_montmul_x": itoa_dmem(x_bytes),
                "_montmul_y": itoa_dmem(y_bytes),
                "_montmul_n": itoa_dmem(n_bytes),
            }
        },
        "output": {
            "dmem": {
                "_montmul_z": itoa_dmem(z_bytes),
            }
        }
    }
